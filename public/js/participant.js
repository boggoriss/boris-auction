...
$(document).ready(function() {
    $(".deletion").on("click", function (event) {
        ...
        console.log(id);
        sendRequest('DELETE', `/api/participant/`, {id})
            .then( r => {
                location.reload()
            })
            .catch( e => {
    ...
    $(".edit").on("click", (event) => {
        ...
        if (changeCash !== null || changeCash !== '') {
            sendRequest('POST', `/api/participant/edit/${id}`, {cashReserve: changeCash})
    ...
    $('#add').on("click", e => {
        ...
        sendRequest('POST', '/api/participant/add', {name, cashReserve})
            .then(r => location.reload())
            .catch(r => location.reload())
    })
});